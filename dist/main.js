...
function datosSegundo(canal) {
    var punto = 22050;
    var segundo = 0;
    var segundos = [];
    while (punto < canal.length - 22050) {
        var forma = getForm(canal[punto], canal[punto + 22050]);
        segundos.push(new individuo_1.individuo(canal[punto], canal[punto + 22050], forma));
        punto += 22050;
        segundo += 0.5;
    }
    return segundos;
}
function comportamiento(canal, tamannoC, http) {
    var puntosCancion = [];
...
function obtenerPuntos(valor1, valor2, forma, cantidadPuntos, puntos, http) {
