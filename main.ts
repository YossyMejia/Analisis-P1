...
function generarMix(cancion:number[]){
    ...
    for(var elemento of cancion){
        var inicio=(elemento-1)*44100;
        var final=(elemento)*44100;
...
function matchDj():number[]{
    ...
    var pos=0;
    while(listaEncoder.length<10 && pos<listaDeS.length){//para obtener el ranking de las tonadas(segundos) mas parecidos
...
function cmpFunction(){
    var datosS1RC = datosSegundo(rigthS1);
    var http = new httpR();
    dataToElastic(datosS1RC,http);
    var puntos = comportamiento(rigthS2,rigthS1.length,http);
    //dataToElastic(rigthS1,http);
    //comportamiento(rigthS2,rigthS1.length);
    //recorrerPuntos(rigthS1);
}

function dataToElastic(segundos:individuo[],http:httpR){
    for(var segundo in segundos){
        var objJson = JSON.parse(JSON.stringify(segundos[segundo]));
        http.uploadIndex(objJson);
    }
}

function getForm(valor1:number,valor2:number):string{ 
    if(Math.abs(valor1)-Math.abs(valor2)>0.01){
        if(valor1>valor2)
            return "BAJADA";
...
function datosSegundo(canal:number[]):individuo[]{
    var punto = 22050;
    var segundo = 0;
    var segundos:individuo[] = [];
    while(punto<canal.length-22050){
        var forma = getForm(canal[punto],canal[punto+22050]);
        segundos.push(new individuo(canal[punto],canal[punto+22050],forma));
        punto+=22050;
        segundo+=0.5;
    }
    return segundos;
...
function comportamiento(canal:number[],tamannoC:number,http:httpR):number[]{
    var puntosCancion:number[] = [];
    var puntosPorPorcion = Math.round(tamannoC/3);
    var tamañoPorcion = Math.round(canal.length/3);
    var punto = 0;
    console.log(canal.length,tamannoC,tamañoPorcion,puntosPorPorcion);
    while(punto<canal.length-12){
        var forma = getForm(canal[punto],canal[punto+11]);
        obtenerPuntos(canal[punto],canal[punto+11],forma,15000,puntosCancion,http);
        punto+=11;
        break;
    }
    return puntosCancion;
}

function obtenerPuntos(valor1:number,valor2:number,forma:string,cantidadPuntos:number,puntos:number[],http:httpR){
    http.search(valor1,valor2,forma);
}
